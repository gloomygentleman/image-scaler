#!/usr/bin/env bash
# AI Image Upscaler (GPU 가속판) — 모델을 캐시에 등록하고,
# 실행 파일과 GPU 런타임(libonnxruntime.dylib)의 Gatekeeper 검역을 제거합니다.
set -euo pipefail
...
echo "AI Image Upscaler (GPU) 설치 도우미"
echo
...
# 2) 폴더 전체의 검역 속성 제거 — 실행 파일 + libonnxruntime.dylib 포함
#    (서명되지 않은 앱/런타임의 Gatekeeper 차단 방지)
...
if [ -f "$DIR/libonnxruntime.dylib" ]; then
  echo "[완료] GPU 런타임 준비: $DIR/libonnxruntime.dylib"
fi

echo
echo "이제 같은 폴더의 image-upscaler 를 실행하세요 (GPU 가속이 자동 적용됩니다)"
